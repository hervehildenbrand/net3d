...
describe('GET /api/meta', () => {
  test('reports backend, version and NAPALM availability', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(res.json()).toEqual({ backend: 'netbox', version: '3.7.8', napalmAvailable: true, layoutEditable: false, layoutPreview: false })
  })
  ...
  test('degrades to no-capabilities instead of failing when the backend is down', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/api/meta' })
    expect(res.statusCode).toBe(200)
    expect(res.json()).toEqual({ backend: 'netbox', version: null, napalmAvailable: false, layoutEditable: false, layoutPreview: false })
  })
})
